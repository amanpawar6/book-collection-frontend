...
const UnreadBooks = () => {
  ...
  // Fetch unread books
  useEffect(() => {
    const fetchUnreadBooks = async (replace = false) => {
      try {
        let headers = {
          Authorization: token
        }
        const response = await getAxiosCall(`/user-book-status/unread?customerId=${user._id}&page=${page}`, { headers });
    ...
    if (user) {
      fetchUnreadBooks(page === 1); // Pass true if it's the first page
    }
  }, [user, page]);
